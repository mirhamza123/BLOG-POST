...
var e = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), e = null), t.exports), t = {
	...
	,
	,
].fill(String.fromCodePoint(n[0])).join(""), a = "\0";
function o(e) {
	let t = JSON.stringify(e), n = new TextEncoder().encode(t), a = "";
...
function l(e, t, n = "auto") {
	return n === !0 || n === "auto" && (s(e) || c(e)) ? e : `${e}${o(t)}`;
}
var u = Object.fromEntries(Object.entries(r).map((e) => [e[1], +e[0]])), d = Object.fromEntries(Object.entries(t).map((e) => e.reverse())), f = `${Object.values(t).map((e) => `\\u{${e.toString(16)}}`).join("")}`, p = RegExp(`[${f}]{4,}`, "gu");
function m(e) {
	let t = e.match(p);
	if (t) return h(t[0], !0)[0];
}
function h(e, t = !1) {
	let n = Array.from(e), r = Infinity, o = -1;
	for (let e = 0; e < n.length; ++e) n[e] === i[0] && n[e + 1] === i[1] && n[e + 2] === i[2] && n[e + 3] === i[3] && (r = Math.min(r, e), o = Math.max(o, e));
	if (o === -1) return g(n, t);
	for (let e = r; e <= o; ++e) if (!((n.length - e) % 4)) try {
		let r = n.slice(e + 4), i = new Uint8Array(r.length / 4);
		for (let e = 0; e < i.length; e++) i[e] = u[r[e * 4]] << 6 | u[r[e * 4 + 1]] << 4 | u[r[e * 4 + 2]] << 2 | u[r[e * 4 + 3]];
		let o = new TextDecoder().decode(i);
		if (t) {
			let e = o.indexOf(a);
			return e === -1 && (e = o.length), [JSON.parse(o.slice(0, e))];
		}
		return o.split(a).filter(Boolean).map((e) => JSON.parse(e));
	} catch (e) {}
	return [];
}
function g(e, t) {
	var n;
	let r = [];
	for (let t = e.length * .5; t--;) {
		let n = `${d[e[t * 2].codePointAt(0)]}${d[e[t * 2 + 1].codePointAt(0)]}`;
		r.unshift(String.fromCharCode(parseInt(n, 16)));
	}
	let i = [], a = [r.join("")], o = 10;
	for (; a.length;) {
		let e = a.shift();
		try {
			if (i.push(JSON.parse(e)), t) return i;
		} catch (t) {
			if (!o--) throw t;
			let r = +((n = t.message.match(/\sposition\s(\d+)$/)) == null ? void 0 : n[1]);
			if (!r) throw t;
			a.unshift(e.substring(0, r), e.substring(r));
		}
	}
	return i;
}
function _(e) {
	...
	return {
		cleaned: e.replace(p, ""),
		encoded: ((t = e.match(p)) == null ? void 0 : t[0]) || ""
	};
}
//#endregion
...
var S = /* @__PURE__ */ e(((e, t) => {
	...
	r.get = function(e, t) {
		...
		var i = Array.isArray(t) ? t : r.parse(t), a = i[0];
		...
		var n = Array.isArray(t) ? t : r.parse(t), i = n[n.length - 1];
...
})))(), w = ({ pointer: e, mappings: t, data: n, hiddenStrings: r }) => {
	let i = t[e];
	if (i) {
		delete t[e];
		let a = E(n, e);
...
var E = (e, t = "") => {
	...
	return n;
}, D = ({ entityId: e, entityType: t, space: n, environment: r, field: i, locale: a, editorInterface: o, fieldType: s, targetOrigin: c, platform: l }) => {
	let u = {
		origin: "contentful.com",
		href: `${`${c || "https://app.contentful.com"}/spaces/${n}/environments/${r}`}/${t === "Entry" ? "entries" : "assets"}/${e}/?focusedField=${i}&focusedLocale=${a}&source=vercel-content-link`,
		contentful: {
			editorInterface: o,
			fieldType: s
		}
	};
	return l === "vercel" && delete u.contentful, u;
}, O = (e) => [
	"builtin",
	"sidebar-builtin",
	"editor-builtin"
].includes(e), k = (e) => F.includes(e), A = [
	"nt_experiences",
	"nt_name",
	"nt_description",
	"nt_audience_id",
	...
	"nt_audience",
	"nt_variants"
], j = A.map((e) => e.replace(/_([a-z])/g, (e, t) => t.toUpperCase())), M = [...A, ...j], N = (e) => M.includes(e);
function P(e) {
...
var L = (e, t, n) => {
	...
	let r = P(e), { spaces: i, environments: a, editorInterfaces: o, fields: s, locales: c, entries: l, assets: u, mappings: d, fieldTypes: f } = r.extensions.contentSourceMaps, p = r;
	for (let e in d) {
		let { source: m } = d[e], h = "entry" in m ? l[m.entry] : u[m.asset], g = "entry" in m ? "Entry" : "Asset";
		if (!h) return r;
		let _ = i[h.space], v = a[h.environment], y = h.id, b = s[m.field], x = c[m.locale], S = o[m.editorInterface], w = f[m.fieldType];
		if (!(O(S.widgetNamespace) && !k(S.widgetId)) && (0, C.has)(p, e)) {
			let r = (0, C.get)(p, e);
			r !== null && I(w, r, D({
				entityId: y,
				entityType: g,
	...
	if (!e.fields) return;
	let { contentSourceMaps: a } = e.sys;
	if (!a) return;
	let { mappings: o } = a;
	for (let a in o) {
		let { source: s } = o[a], c = e.sys.space.sys.id, l = e.sys.environment.sys.id, u = e.sys.id, d = e.sys.type, f = t[s.fieldType], p = n[s.editorInterface];
	...
	if (r.sys && "items" in r) {
		...
		let { contentSourceMapsLookup: { fieldTypes: a, editorInterfaces: o } } = e.sys, { items: s, includes: c } = e;
...
//#endregion
export { F as SUPPORTED_WIDGETS, P as clone, v as combine, D as createSourceMapMetadata, b as decode, y as encode, z as encodeCPAResponse, I as encodeField, L as encodeGraphQLResponse, w as encodeRichTextValue, O as isBuiltinNamespace, N as isNinetailedField, k as isSupportedWidget, x as splitEncoding };
